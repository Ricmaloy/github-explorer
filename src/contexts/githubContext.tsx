...
const GithubProvider: FC = ({ children }) => {
    ...
    const searchUser = async (user: string) => {
        const userData = await fetch(`https://api.github.com/users/${user}`).then((response) => response.json());
    ...
    const searchUserRepos = async (user: string) => {
        const userReposData = await fetch(`https://api.github.com/users/${user}/repos`).then((response) => response.json());
    ...
    return ( 
        <GithubContext.Provider value={{user , repositories, clearInfos, searchUser, searchUserRepos  }} >
            {children}
